...
% 	bool : boolean
% 
% Test if there is any complex number in the implicit fractionnal
% polynomial
% 
%   Copyright © CRONE

%% Function-based unit test for frac_fpi_iscomplex
% Last revision 09/09/2021

function tests = test_fpi_iscomplex
...
function testIsComplexSISO(testCase)
    ...
    polcmplx1=frac_poly_imp(frac_poly_exp([1,2+1i,4],[3,2,0]),3);
    polcmplx1bis=frac_poly_imp(frac_poly_exp([1,2,4],[3,2,0]),3+1i);
...
% iscomplex(polcmplx1bis) returns 1
	assertTrue(testCase,iscomplex(polcmplx1bis));
end
%% The classic case : multidimensionnal
function testIsComplexMIMO(testCase)
    pol1=frac_poly_imp(frac_poly_exp([1,2,4],[3,2,0]),3);
    polcmplx1=frac_poly_imp(frac_poly_exp([1,2+1i,4],[3,2,0]),3);
