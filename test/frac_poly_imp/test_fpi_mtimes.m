...
% 
% Realize the operation fpe1 x fp2.
...
%% Function-based unit test for frac_fpi_mtimes
% Last revision 09/09/2021

function tests = test_fpi_mtimes
...
% multiplication with a non null implicit order won't work
	assertEqual(testCase,mtimes(pol1,pol1),pol1_2,'mtimes(pol1,pol1) is not pol1^2');
    assertEmpty(testCase,mtimes(pol1bis,pol1bis),'mtimes(pol1bis,pol1bis) is not empty');
end
%% The classic case : multidimensionnal
function testMtimesMIMO(testCase)
    pol1=frac_poly_imp([1,2,4],[3,2,0],1);
    pol1bis=frac_poly_imp([3,12,2],[3,1,0],3);
...
% multiplication.
    ...
    assertEqual(testCase,mtimes(pol3,pol3),pol3_exp,'mtimes(pol3,pol3) is not pol3_exp');
    assertEmpty(testCase,mtimes(pol3bis,pol3bis),'mtimes(pol3bis,pol3bis) is not empty');
end
